...
namespace TLLib
{
    public class ProductColor
    {
        ...
        public DataTable ProductColorSelectAll2(string ProductColorName, string IsAvailable, string ProductID)
        {
            DataTable table2;
            try
            {
                DataTable dataTable = new DataTable();
                SqlConnection connection = new SqlConnection(this.connectionString);
                SqlCommand selectCommand = new SqlCommand("usp_ProductColor_SelectAll2", connection)
                {
                    CommandType = CommandType.StoredProcedure
                };
                selectCommand.Parameters.AddWithValue("@ProductColorName", string.IsNullOrEmpty(ProductColorName) ? ((object)this.dbNULL) : ((object)ProductColorName));
                selectCommand.Parameters.AddWithValue("@IsAvailable", string.IsNullOrEmpty(IsAvailable) ? ((object)this.dbNULL) : ((object)IsAvailable));
                selectCommand.Parameters.AddWithValue("@ProductID", string.IsNullOrEmpty(ProductID) ? ((object)this.dbNULL) : ((object)ProductID));
                SqlParameter parameter = new SqlParameter("@ErrorCode", null)
                {
                    Size = 4,
                    Direction = ParameterDirection.Output
                ...
                if (parameter.Value.ToString() != "0")
                {
                    throw new Exception("Stored Procedure 'usp_ProductColor_SelectAll2' reported the ErrorCode : " + parameter.Value.ToString());
                }
                table2 = dataTable;
            }
            catch (Exception exception)
